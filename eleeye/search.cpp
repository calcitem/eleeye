    ...
    if (Search.bNullMove && !bNoNull && Search.pos.LastMove().ChkChs <= 0 && Search.pos.NullOkay()) {
        ...
        if (vl >= vlBeta) {
            if (Search.pos.NullSafe()) {
                ...
                RecordHash(Search.pos, HASH_BETA, vl, std::max(nDepth, NULL_DEPTH + 1), 0);
                return vl;
            } else if (SearchCut(vlBeta, nDepth - NULL_DEPTH, NO_NULL) >= vlBeta) {
                // b. 如果空着裁剪带检验，那么记录深度至少为(NULL_DEPTH);
                RecordHash(Search.pos, HASH_BETA, vl, std::max(nDepth, NULL_DEPTH), 0);
                return vl;
            }
        }
    }
    ...
    while ((mv = Search2.MoveSort.NextRoot()) != 0) {
        if (Search.pos.MakeMove(mv)) {
            ...
            if (vl > vlBest) {
                ...
                if (vlBest > -WIN_VALUE && vlBest < WIN_VALUE) {
                    vlBest += (Search.rc4Random.NextLong() & Search.nRandomMask) - (Search.rc4Random.NextLong() & Search.nRandomMask);
